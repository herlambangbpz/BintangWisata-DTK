...
const AirlineOrderBaggage: React.FC<AirlineOrderBaggageProps> = ({
  ...
  const [BaggageStatus, setBaggageStatus] = useState<any>(null);
      ...
      if (
        ...
        if (nulldata) {
          if (baggages !== null) {
            const codeData =
              baggages[0] &&
              baggages[0].baggageInfos.sort((a, b) => a.fare - b.fare)[0];
            if (codeData) {
              dv[0].forEach((i) => {
                dataone.push(codeData);
              });
            }
          }
          if (baggages !== null && baggages.length > 1) {
            ...
            if (codeData) {
              dv[1].forEach((i) => {
                datatwo.push(codeData);
              });
            }
          }
          if (datatwo.length > 0) {
            arraydata = [dataone, datatwo];
          } else {
            arraydata = [dataone];
          }
  ...
  return (
    <div
      ...
      hidden={BaggageStatus === "notavailable" ? true : false}
    >
          ...
          <IonGrid>
            <IonRow>
              <IonCol size="10">
                ...
                <IonText
                  color={BaggageStatus === "available" ? "dark" : "medium"}
                >
                  {BaggageStatus === "available"
                    ? "Bagasi"
                    : "Menunggu Data Penumpang"}
                </IonText>
              </IonCol>
